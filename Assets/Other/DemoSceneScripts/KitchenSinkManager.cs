...
public class KitchenSinkManager : MonoBehaviour
{
	...
	private IEnumerator animatePanel( UIObject sprite )
	{
		while( true )
		{
			...
			var ani = sprite.positionTo( 0.7f, new Vector3( 200f, 0, 1 ), UIEasing.Quartic.easeIn );
	...
	private IEnumerator animatePosition( UISprite sprite, Vector3 to, float duration )
	{
		...
		var ani = sprite.positionTo( duration, to, UIEasing.Quartic.easeInOut );
		ani.autoreverse = true;
	...
	private IEnumerator animateRotation( UISprite sprite, Vector3 to, float duration )
	{
		...
		var ani = sprite.eulerAnglesTo( duration, to, UIEasing.Sinusoidal.easeOut );
		yield return ani.chain();

		sprite.eulerAnglesTo( duration, originalPosition, UIEasing.Circular.easeIn );
	}
	...
	private IEnumerator pulseOptionButton( UIButton optionsButton )
	{
		...
		while( true )
		{
			ani = optionsButton.alphaTo( 0.7f, 0.1f, UIEasing.Linear.easeIn );
			yield return ani.chain();
			
			ani = optionsButton.alphaTo( 0.7f, 1.0f, UIEasing.Linear.easeOut );
			yield return ani.chain();
		}
	}
	...
	{
		var ani = sender.scaleFromTo( 0.3f, new Vector3( 1, 1, 1 ), new Vector3( 1.3f, 1.3f, 1 ), UIEasing.Sinusoidal.easeOut );
		ani.autoreverse = true;
